import { Box, Button, Flex, Text, Progress, VStack, HStack, Badge } from "@chakra-ui/react";
...
const PomodoroTimer = () => {
  ...
  useEffect(() => {
    if (session.isRunning && session.timeRemaining > 0) {
      timerRef.current = window.setInterval(() => {
        setSession(prev => ({ ...prev, timeRemaining: prev.timeRemaining - 1000 }));
      }, 1000);
  ...
  const handlePhaseComplete = () => {
    ...
    setSession(prev => {
      ...
      if (prev.phase === 'work') {
        ...
        nextPhase = nextCycleCount % prev.totalCycles === 0 ? 'longBreak' : 'shortBreak';
      } else {
  ...
  const handleStartStop = async () => {
    ...
    if (!session.isRunning) {
      ...
      if (session.phase === 'work' && session.timeRemaining === POMODORO_TIMES.work) {
        startAlarm();
  ...
  const getPhaseColor = (phase: PomodoroPhase) => {
    switch (phase) {
      case 'work': return 'red.500';
      case 'shortBreak': return 'green.500';
      case 'longBreak': return 'blue.500';
    }
  };
  ...
  const progressPercent = ((POMODORO_TIMES[session.phase] - session.timeRemaining) / POMODORO_TIMES[session.phase]) * 100;
  const completedWorkSessions = session.phase === 'work' ? session.cycleCount : session.cycleCount;
  ...
  return (
    <Box 
      ...
      sx={{
        ...
        right: 0,
        bottom: 0,
        backgroundColor: isFlashing ? getPhaseColor(session.phase) : 'transparent',
    ...
    >
      <VStack spacing={6} maxW="500px" w="100%">
        ...
        <VStack spacing={2}>
          <HStack spacing={6}>
            <VStack spacing={1}>
              <Text fontSize="2xl" fontWeight="700" color={getPhaseColor(session.phase)}>
